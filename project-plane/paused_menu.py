...
import os
class paused:
    def __init__(self,highscore,screen,abilityselection,replitbad,overallsound) -> None:
        self.screen=screen
        ...
        self.quit=button(screen.get_width()//2,screen.get_height()//2,self.quit_text,overallsound,replitbad,screen)
        ...
        self.coin_text=self.small_font.render(self.coin_gotten,True,(255,255,255))
        self.coin_image=pygame.transform.scale(self.coin_image,(self.coin_image.get_width(),self.coin_text.get_height()))
        self.score=self.sfont.render(f"score:{highscore}",True,(255,255,255))

    def draw(self,highscore):
        self.quit.text(self.screen.get_width()//2-self.quit.rect.w//2,self.screen.get_height()//2+self.abilityselection.bigbox.y)
        ...
        self.coin_text=self.small_font.render(f"{self.coin_gotten} + {self.highscore//10}",True,(255,255,255))
        self.coin_image=pygame.transform.scale(self.coin_image,(self.coin_image.get_width(),self.coin_text.get_height()))
        self.screen.blit(self.coin_image,(self.screen.get_width()//2-self.coin_text.get_width()-self.coin_image.get_width()//2,self.screen.get_height()//2+self.paused_text.get_height()))
        self.screen.blit(self.coin_text,(self.screen.get_width()//2-self.coin_text.get_width()//2,self.screen.get_height()//2+self.paused_text.get_height()))
