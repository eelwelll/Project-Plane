...
class shop_button:
    def __init__(self,x,y,content,image,description,pos,cost,actual_name) -> None:
        ...
        while let!=len(self.desc_get_text):
            ...
            if (pygame.font.Font.size(self.sfont,self.desc_get_text[last_let_place:let])[0])>=self.text_box_description.w-self.text_box_description.w//4:
                if not end:
                    ...
                    self.desc_text.append((self.desc_get_text[last_let_place:des],self.text_box_description.y+5+self.image.get_height()+5+(height*len(self.desc_text))))
    ...
    def check(self,objective,position):
        ...
        if pygame.mouse.get_pressed()[0] and objective and self.rect.collidepoint(mouse):
                    ...
                    for line in have:
                        coin=int(line)
                    print(self.cost_number>=coin)


                    if self.gotten == "False" and not self.cost_number>=coin:
                            ...
                            actualshop.truncate()
                            whatwewrite=""
                            for pos,l in enumerate(shop_contain.shop):
                                    ...
                                    if l[0]==self.what:
                                        ...
                                        whatwewrite+=f"{shop_contain.shop[pos][3]}\n"
...
class shoop:
    ...
    def shop_maker(self):
            ...
            for pos,items in enumerate(shop_items):
                ...
                self.listofitems.append((shop_button(xofbox,yofbox,item_name,items[1],items[2],pos,items[4],items[0]),(xofbox,yofbox),line_get)) # x,y,content,image,description,if they got the ability,cost
